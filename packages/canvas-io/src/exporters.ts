...
export function measureSkedraExportBounds(sourceLayer: SVGGElement) {
	const uiElements = Array.from(
		sourceLayer.querySelectorAll<SVGElement>(EXPORT_UI_SELECTOR),
	);
	const previousDisplays = uiElements.map((element) => element.style.display);
	try {
		for (const element of uiElements) element.style.display = "none";
		return sourceLayer.getBBox();
	} finally {
		for (const [index, element] of uiElements.entries()) {
			const display = previousDisplays[index];
